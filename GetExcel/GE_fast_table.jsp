...
<%
String sql = "select TT.business_unit_source,a.apply_num,b.pass_num,c.refuse_num,to_char(round(b.pass_num*100/(b.pass_num+c.refuse_num),2),'990.99')||'%' as pass_rate,d.loan_num,d.loan_money,round(loan_money/loan_num,2) as ave from (select distinct business_unit_source from xndb.apply_info where business_unit_source^='APP服务' and business_unit_source^='首付贷事业部' and business_unit_source is not null) TT,(select b.business_unit_source,count(a.apply_id) as apply_num from (select a.APPLY_ID from xndb.app_result a where a.ACTIVITY_ID='utask_apply_review' group by a.APPLY_ID having to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual)) a left join xndb.apply_info b on a.apply_id=b.apply_id group by b.business_unit_source) a,(select b.business_unit_source,count(a.apply_id) as pass_num from (select a.APPLY_ID from xndb.app_result a where (a.ACTIVITY_ID='utask_check2' and a.result='PASS' and a.handl_time is not null) or (a.ACTIVITY_ID='utask_check3' and a.result='PASS' and a.handl_time is not null)  group by a.APPLY_ID having to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual)) a left join xndb.apply_info b on a.apply_id=b.apply_id group by b.business_unit_source) b,(select b.business_unit_source,count(a.apply_id) as refuse_num from (select a.APPLY_ID,to_char(max(a.HANDL_TIME),'YYYY/MM/DD') as pass_date from xndb.app_result a where (a.ACTIVITY_ID='utask_check2' and a.result='REFUSE' and a.handl_time is not null) or (a.ACTIVITY_ID='utask_check3' and a.result='REFUSE' and a.handl_time is not null)  group by a.APPLY_ID having to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual)) a left join xndb.apply_info b on a.apply_id=b.apply_id group by b.business_unit_source) c,(select b.business_unit_source,count(a.contract_no) as loan_num,round(sum(a.CONTRACT_AMOUNT)/10000,0) as loan_money from xndb.apply_contract a left join xndb.apply_info b on a.apply_id=b.apply_id where a.contract_state in('PASS','STPASS','STFAILER') and to_char(a.real_pay_date,'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual) group by b.business_unit_source) d where tt.business_unit_source=a.business_unit_source and tt.business_unit_source=b.business_unit_source and tt.business_unit_source=c.business_unit_source and tt.business_unit_source=d.business_unit_source union all select '小牛普惠',sum(apply_num),sum(pass_num),sum(refuse_num),to_char(round(sum(pass_num)*100/(sum(pass_num)+sum(refuse_num)),2),'990.99')||'%',sum(loan_num),sum(loan_money),round(sum(loan_money)/sum(loan_num),2) from (select TT.business_unit_source,a.apply_num,b.pass_num,c.refuse_num,to_char(round(b.pass_num*100/(b.pass_num+c.refuse_num),2),'990.99')||'%' as pass_rate,d.loan_num,d.loan_money,round(loan_money/loan_num,2) as ave from (select distinct business_unit_source from xndb.apply_info where business_unit_source^='APP服务' and business_unit_source^='首付贷事业部' and business_unit_source is not null) TT,(select b.business_unit_source,count(a.apply_id) as apply_num from (select a.APPLY_ID from xndb.app_result a where a.ACTIVITY_ID='utask_apply_review' group by a.APPLY_ID having to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual)) a left join xndb.apply_info b on a.apply_id=b.apply_id group by b.business_unit_source) a,(select b.business_unit_source,count(a.apply_id) as pass_num from (select a.APPLY_ID from xndb.app_result a where (a.ACTIVITY_ID='utask_check2' and a.result='PASS' and a.handl_time is not null) or (a.ACTIVITY_ID='utask_check3' and a.result='PASS' and a.handl_time is not null)  group by a.APPLY_ID having to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual)) a left join xndb.apply_info b on a.apply_id=b.apply_id group by b.business_unit_source) b,(select b.business_unit_source,count(a.apply_id) as refuse_num from (select a.APPLY_ID,to_char(max(a.HANDL_TIME),'YYYY/MM/DD') as pass_date from xndb.app_result a where (a.ACTIVITY_ID='utask_check2' and a.result='REFUSE' and a.handl_time is not null) or (a.ACTIVITY_ID='utask_check3' and a.result='REFUSE' and a.handl_time is not null)  group by a.APPLY_ID having to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual)) a left join xndb.apply_info b on a.apply_id=b.apply_id group by b.business_unit_source) c,(select b.business_unit_source,count(a.contract_no) as loan_num,round(sum(a.CONTRACT_AMOUNT)/10000,0) as loan_money from xndb.apply_contract a left join xndb.apply_info b on a.apply_id=b.apply_id where a.contract_state in('PASS','STPASS','STFAILER') and to_char(a.real_pay_date,'YYYY/MM/DD')>=(select to_char(TRUNC(SYSDATE, 'MM'),'YYYY/MM/DD')from dual) group by b.business_unit_source) d where tt.business_unit_source=a.business_unit_source and tt.business_unit_source=b.business_unit_source and tt.business_unit_source=c.business_unit_source and tt.business_unit_source=d.business_unit_source) order by apply_num desc";
...
try {	
	...
	cell0.setCellValue("BUSINESS_UNIT_SOURCE");
	...
	while(rs.next()){
		...
		celli0.setCellValue(rs.getString("BUSINESS_UNIT_SOURCE"));
	...
	response.setContentType("application/vnd.ms-excel; charset=utf-8");
	 response.setHeader("Content-Disposition","attachment;filename=ph_fast_table.xlsx");
