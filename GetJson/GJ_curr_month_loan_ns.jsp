...
<%
String sql = "select a.loan_date,a.loan_money,b.loan_money1,c.loan_money2 from (select to_char(a.real_pay_date,'YYYY/MM/DD') as loan_date,round(sum(a.CONTRACT_AMOUNT)/10000,0) as loan_money from xndb.apply_contract a left join xndb.apply_info b on a.apply_id=b.apply_id where a.contract_state in('PASS','STPASS','STFAILER') and b.BUSINESS_UNIT_SOURCE='农商贷事业部' and b.region_area_ is not null group by to_char(a.real_pay_date,'YYYY/MM/DD') having to_char((select sysdate - interval '30' day from dual),'YYYY/MM/DD')<to_char(a.real_pay_date,'YYYY/MM/DD')) a left join (select to_char(a.real_pay_date,'YYYY/MM/DD') as loan_date,round(sum(a.CONTRACT_AMOUNT)/10000,0) as loan_money1 from xndb.apply_contract a left join xndb.apply_info b on a.apply_id=b.apply_id where a.contract_state in('PASS','STPASS','STFAILER') and b.BUSINESS_UNIT_SOURCE='农商贷事业部' and b.region_area_ is not null and b.region_area_='农商一区' group by to_char(a.real_pay_date,'YYYY/MM/DD') having to_char((select sysdate - interval '30' day from dual),'YYYY/MM/DD')<to_char(a.real_pay_date,'YYYY/MM/DD')) b on a.loan_date=b.loan_date left join (select to_char(a.real_pay_date,'YYYY/MM/DD') as loan_date,round(sum(a.CONTRACT_AMOUNT)/10000,0) as loan_money2 from xndb.apply_contract a left join xndb.apply_info b on a.apply_id=b.apply_id where a.contract_state in('PASS','STPASS','STFAILER') and b.BUSINESS_UNIT_SOURCE='农商贷事业部' and b.region_area_ is not null and b.region_area_='农商二区' group by to_char(a.real_pay_date,'YYYY/MM/DD') having to_char((select sysdate - interval '30' day from dual),'YYYY/MM/DD')<to_char(a.real_pay_date,'YYYY/MM/DD')) c on a.loan_date=c.loan_date order by loan_date";/*存储sql语句*/
