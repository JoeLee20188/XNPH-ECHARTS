...
<%
String sql="select x.BUSINESS_UNIT_SOURCE,x.apply_num,y.loan_num,y.loan_money from (select b.apply_date,count(b.apply_id)as apply_num,c.BUSINESS_UNIT_SOURCE from (select a.APPLY_ID,to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD') as apply_date from xndb.app_result a where a.ACTIVITY_ID='utask_apply_review' group by a.APPLY_ID) b left join xndb.apply_info c on b.APPLY_ID=c.APPLY_ID where b.apply_date=to_char((select sysdate from dual),'YYYY/MM/DD') group by b.apply_date,c.BUSINESS_UNIT_SOURCE) x left join (select b.loan_date,c.BUSINESS_UNIT_SOURCE,count(b.contract_no) as loan_num,round(sum(b.CONTRACT_AMOUNT)/10000,0) as loan_money from (select a.apply_id,a.contract_no,to_char(a.real_pay_date,'YYYY/MM/DD') as loan_date,a.CONTRACT_AMOUNT from xndb.apply_contract a where a.contract_state in('PASS','STPASS','STFAILER')) b left join xndb.apply_info c on b.APPLY_ID=c.APPLY_ID where loan_date=to_char((select sysdate from dual),'YYYY/MM/DD') group by b.loan_date,c.BUSINESS_UNIT_SOURCE) y on x.BUSINESS_UNIT_SOURCE=y.BUSINESS_UNIT_SOURCE union all select '小牛普惠',sum(apply_num),sum(loan_num),sum(loan_money) from (select x.BUSINESS_UNIT_SOURCE,x.apply_num,y.loan_num,y.loan_money from (select b.apply_date,count(b.apply_id)as apply_num,c.BUSINESS_UNIT_SOURCE from (select a.APPLY_ID,to_char(MIN(a.HANDL_TIME),'YYYY/MM/DD') as apply_date from xndb.app_result a where a.ACTIVITY_ID='utask_apply_review' group by a.APPLY_ID) b left join xndb.apply_info c on b.APPLY_ID=c.APPLY_ID where b.apply_date=to_char((select sysdate from dual),'YYYY/MM/DD') group by b.apply_date,c.BUSINESS_UNIT_SOURCE) x left join (select b.loan_date,c.BUSINESS_UNIT_SOURCE,count(b.contract_no) as loan_num,round(sum(b.CONTRACT_AMOUNT)/10000,0) as loan_money from (select a.apply_id,a.contract_no,to_char(a.real_pay_date,'YYYY/MM/DD') as loan_date,a.CONTRACT_AMOUNT from xndb.apply_contract a where a.contract_state in('PASS','STPASS','STFAILER')) b left join xndb.apply_info c on b.APPLY_ID=c.APPLY_ID where loan_date=to_char((select sysdate from dual),'YYYY/MM/DD') group by b.loan_date,c.BUSINESS_UNIT_SOURCE) y on x.BUSINESS_UNIT_SOURCE=y.BUSINESS_UNIT_SOURCE) order by apply_num";
