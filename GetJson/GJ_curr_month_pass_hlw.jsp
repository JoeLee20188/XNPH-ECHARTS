...
<%
String sql = "select a.pass_date,a.pass_num,b.pass_num1 from (select count(a.apply_id) as pass_num,a.pass_date from (select a.APPLY_ID,to_char(max(a.HANDL_TIME),'YYYY/MM/DD') as pass_date from xndb.app_result a left join xndb.apply_info b on a.apply_id=b.apply_id where (a.ACTIVITY_ID='utask_check2' and a.result='PASS' and a.handl_time is not null and b.BUSINESS_UNIT_SOURCE='互联网服务事业部' and b.region_area_ is  not null) or (a.ACTIVITY_ID='utask_check3' and a.result='PASS' and a.handl_time is not null and b.BUSINESS_UNIT_SOURCE='互联网服务事业部' and b.region_area_ is  not null) group by a.APPLY_ID having to_char((select sysdate - interval '30' day from dual),'YYYY/MM/DD')<to_char(max(a.HANDL_TIME),'YYYY/MM/DD')) a group by a.pass_date) a left join (select count(a.apply_id) as pass_num1,a.pass_date from (select a.APPLY_ID,to_char(max(a.HANDL_TIME),'YYYY/MM/DD') as pass_date from xndb.app_result a left join xndb.apply_info b on a.apply_id=b.apply_id where (a.ACTIVITY_ID='utask_check2' and a.result='PASS' and a.handl_time is not null and b.BUSINESS_UNIT_SOURCE='互联网服务事业部' and b.region_area_ is  not null and b.region_area_='华南区') or (a.ACTIVITY_ID='utask_check3' and a.result='PASS' and a.handl_time is not null and b.BUSINESS_UNIT_SOURCE='互联网服务事业部' and b.region_area_ is  not null and b.region_area_='华南区') group by a.APPLY_ID,b.region_area_ having to_char((select sysdate - interval '30' day from dual),'YYYY/MM/DD')<to_char(max(a.HANDL_TIME),'YYYY/MM/DD')) a group by a.pass_date) b on a.pass_date=b.pass_date order by pass_date";/*存储sql语句*/
